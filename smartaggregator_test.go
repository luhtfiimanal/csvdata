...
func TestDataMean(t *testing.T) {
	...
	reqcolumn := csvdata.SAColumn{
		...
		WindowRelative:   [][2]int64{{2, 3}, {4, 6}, {8, 10}, {11, 13}, {14, 16}, {17, 19}},
	...
	expected := []float64{2.5, 5.0, 9.0, 12.0, 15.0, 18.0}
...
func TestDataPick(t *testing.T) {
	...
	timeResultEp := []int64{2, 5, 7, 8, 10}

	reqcolumn := csvdata.SAColumn{
		OutputColumnName: "dewpoint_pick",
		PickRelative:     []int64{2, 5, 7, 8, 10},
		TimeResultEp:     &timeResultEp,
		Result:           make([]float64, len(timeResultEp)),
	}
	...
	expected := []float64{2.0, 4.0, 7.0, 8, 10}
